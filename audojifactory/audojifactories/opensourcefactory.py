...
from audojiengine_sm.logging_config import configure_logger
from audojiengine_sm.mg_database import store_data_to_audio_segment_mgdb
...
class AudioProcessor:
    ...
    async def transcribe_audio_sagemaker(self):
        sagemaker_runtime = boto3.client("sagemaker-runtime", region_name="your-region")

        # Load your audio file content
        with open(self.audio_path, "rb") as f:
            payload = f.read()

        response = sagemaker_runtime.invoke_endpoint(
            EndpointName="your-sagemaker-endpoint-name",
            ContentType="application/x-audio",  # Adjust based on the expected content type
            Body=payload,
        )
